...
TEST_CASE("Partitiontree creates and gets correct buckets", "[partitiontree]") {
    ...
    SECTION("Fine-grained buckets are created when necessary") {
        ...
        REQUIRE(lvl0.getBounds() == Bounds(Point(0, 0), Point(4, 4)));
        REQUIRE(lvl1.getBounds() == Bounds(Point(0, 0), Point(2, 2)));
        REQUIRE(lvl2.getBounds() == Bounds(Point(0, 0), Point(1, 1)));

        REQUIRE(lvl0.getLevel() == 0);
        REQUIRE(lvl1.getLevel() == 1);
        REQUIRE(lvl2.getLevel() == 2);
    ...
    SECTION ("If a bucket at a specified level does not exist, a containing bucket is returned") {
        ...
        REQUIRE(lvl0.getBounds() == Bounds(Point(0, 0), Point(4, 4)));
        REQUIRE(lvl1.getBounds() == Bounds(Point(2, 2), Point(4, 4)));
        REQUIRE(lvl2.getBounds() == Bounds(Point(2, 2), Point(4, 4)));

        REQUIRE(lvl0.getLevel() == 0);
        REQUIRE(lvl1.getLevel() == 1);
        REQUIRE(lvl2.getLevel() == 1);
