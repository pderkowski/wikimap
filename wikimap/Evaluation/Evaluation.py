...
def check(embeddings):
    datasetPath = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'datasets', 'EN-WS-353-ALL.txt')
    with open(datasetPath, 'r') as dataset:
        missing = 0
        for i, line in enumerate(dataset):
            words = line.strip().split()
            if not embeddings.has(words[0]):
                print 'Line {}: {}'.format(i, words[0])
                missing += 1
            if not embeddings.has(words[1]):
                print 'Line {}: {}'.format(i, words[1])
                missing += 1

        print 'There were {} missing words.'.format(missing)
