...
class ComputeEmbeddings(Job):
    def __init__(self, node_count, dimensions, context_size, backtrack_probability):
        super(ComputeEmbeddings, self).__init__('COMPUTE EMBEDDINGS', alias='embed',
            inputs=[P.link_edges, P.pagerank], outputs=[P.embeddings],
            node_count=node_count, dimensions=dimensions, context_size=context_size, backtrack_probability=backtrack_probability)

    def __call__(self, node_count, dimensions, context_size, backtrack_probability):
        edges = self.data.get_link_edges_between_highest_ranked_nodes(node_count)
        embeddings = Node2Vec(edges, dims=dimensions, context_size=context_size, backtrack_prob=backtrack_probability)
